...
export const GLM_EMBEDDING_MODEL = process.env.ZHIPU_EMBEDDING_MODEL ?? 'text_embedding'
...
export function defaultVectorApiBase(): string {
  return `http://127.0.0.1:${process.env.AGENT_MYSQL_SERVICE_PORT ?? '37123'}`
}
...
/** 本地向量 / 文档 API 默认超时（ms） */
export const RAG_DEFAULT_SERVE_TIMEOUT_MS = 20_000

/** 智谱 embedding 请求默认超时（ms） */
export const RAG_DEFAULT_EMBEDDING_TIMEOUT_MS = 15_000

/** 入库时单次请求智谱 embedding 的批大小 */
export const RAG_INGEST_EMBEDDING_BATCH_SIZE = 8
