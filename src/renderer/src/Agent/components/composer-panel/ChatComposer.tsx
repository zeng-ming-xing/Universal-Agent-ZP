/**
 * 聊天输入框组件
 * 含 textarea（Enter 发送 / Shift+Enter 换行）、发送/停止按钮、免责声明
 */
...
const ChatComposer = ({ value, onChange, onSend, onAbort, sending = false }: ChatComposerProps) => {
    ...
    <footer>
      <div className="focus-within:ring-gray-200 relative flex flex-col rounded-3xl bg-[#f4f4f4] px-4 py-3 focus-within:ring-1">
        ...
        <div className="absolute bottom-3 right-3 flex items-center gap-2">
          ...
          <button
            type="button"
            className={`flex h-8 w-8 items-center justify-center rounded-full transition ${canSend
